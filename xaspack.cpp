...
bool decryptStubHead(quint32 nImageBase, quint32 nEp, quint32 nLandingOff, quint8 *image, quint32 nImageSize, XBinary::PDSTRUCT *pPdStruct)
{
    ...
    if (!memory.mapFixed(nImageBase, nImageSize, rwx, QStringLiteral("ASPack image")) ||
        !memory.mapFixed(ASP_STACK, ASP_STACK_SIZE, rw, QStringLiteral("ASPack stack")) || !memory.write(nImageBase, image, nImageSize)) {
        return false;
    }
...
const quint8 g_aspackCompBTable[0x72] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x0a, 0x0c, 0x0e, 0x10, 0x14, 0x18, 0x1c, 0x20, 0x28, 0x30,
                                         0x38, 0x40, 0x50, 0x60, 0x70, 0x80, 0xa0, 0xc0, 0xe0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01,
                                         0x01, 0x01, 0x02, 0x02, 0x02, 0x02, 0x03, 0x03, 0x03, 0x03, 0x04, 0x04, 0x04, 0x04, 0x05, 0x05, 0x05, 0x05, 0x00,
                                         0x00, 0x00, 0x00, 0x01, 0x01, 0x02, 0x02, 0x03, 0x03, 0x04, 0x04, 0x05, 0x05, 0x06, 0x06, 0x07, 0x07, 0x08, 0x08,
                                         0x09, 0x09, 0x0a, 0x0a, 0x0b, 0x0b, 0x0c, 0x0c, 0x0d, 0x0d, 0x0e, 0x0e, 0x0f, 0x0f, 0x10, 0x10, 0x11, 0x11, 0x11,
                                         0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x12, 0x12, 0x12, 0x12, 0x12, 0x12, 0x12, 0x12};
...
int XASPACK::_buildDicts(ASPK *s)
{
    ...
    while (counter < 757) {
        ...
        if (ret >= 16) {
            if (ret != 16) {
                if (ret == 17) ret = 3 + _getbits(s, 3, &oob);
                else ret = 11 + _getbits(s, 7, &oob);
                if (oob) return 0;
                while (ret) {
...
bool XASPACK::_unpackToBuffer(QByteArray &baOut, qint64 nOutputLimit, PDSTRUCT *pPdStruct)
{
    ...
    if (!nImageSize || (nImageSize > (256u * 1024 * 1024)) || ((nOutputLimit >= 0) && ((quint64)nImageSize > (quint64)nOutputLimit))) return false;
